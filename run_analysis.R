...
# It has been raised here - https://r-forge.r-project.org/tracker/index.php?func=detail&aid=5413&group_id=240&atid=975
#
...
row2 <- nrow(dataTable[, .N, by=c("featDomain", "featAcceleration", "featInstrument", "featJerk", "featMagnitude", "featVariable", "featAxis")])
row1 == row2
...
dtTidy <- dataTable[, list(count = .N, average = mean(value)), by=key(dataTable)]

write.csv(dtTidy, "output.csv")